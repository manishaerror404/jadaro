const products = [
    ...
    const state = { category: "all", style: "all", finish: "all", search: "", sort: "featured", cart: 0 };
        ...
        <article class="card product reveal visible">
          ...
          <div class="product-body">
            ...
            <div class="product-actions">
              ...
              <button class="btn btn-primary" type="button" data-add="${index}">${icon("bagPlus")} Add</button>
            </div>
          </div>
        </article>
      `).join("");
    ...
    function openModal(product) {
      ...
      document.getElementById("modalAdd").onclick = addToCart;
      ...
      document.getElementById("saleProducts").innerHTML = saleProducts.slice(0, 3).map((product, index) => `
        <article class="sale-product">
          ...
          <div>
            ...
            <button class="btn btn-ghost" type="button" data-sale-add="${index}">${icon("bagPlus")} Add to Bag</button>
          </div>
        </article>
      `).join("");
    }
    ...
    document.getElementById("saleProducts").addEventListener("click", event => {
      if (event.target.closest("[data-sale-add]")) addToCart();
    });
